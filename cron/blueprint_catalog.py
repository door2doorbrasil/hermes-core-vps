...
CATALOG: List[AutomationBlueprint] = [
    ...
    AutomationBlueprint(
        key="buyer-outreach-30d",
        title="Buyer outreach follow-up",
        description="A 30-day recurring outreach batch for qualified buyers "
        "and potential importers in the destination markets.",
        category="sales",
        schedule_template="every 30d",
        prompt_template=(
            "Prepare the next sales outreach batch for qualified buyers and "
            "potential importers tied to the current Hermes Vendas profile. "
            "Use only role-bound lookup and internal commercial data; do not "
            "do generic web search. Focus on Polar Sinergy's product line: "
            "bicarbonato de sódio, bicarbonato de amônio, bicarbonato de "
            "potássio, sagu, fécula de batata, fécula de mandioca, polvilho "
            "doce and polvilho azedo. Draft short, transacionally useful "
            "messages that highlight the product-specific differentiators "
            "that improve close probability in the target market. Route the "
            "batch through the approval queue before any real send. If "
            "nothing new should be sent, respond with [SILENT]."
        ),
        slots=[_DELIVER],
        skills=("hermes-mail-commercial-operations", "hermes-vendas"),
        tags=("sales", "outreach", "buyers"),
    ),
    AutomationBlueprint(
        key="secom-buyer-report-request",
        title="SECOM buyer-report request",
        description="A 45-day recurring request to registered SECOM offices "
        "for importer and buyer reports in destination countries.",
        category="sales",
        schedule_template="every 45d",
        prompt_template=(
            "Using the SECOM offices already registered in the system, prepare "
            "the next request for importer and buyer reports for the same "
            "Polar Sinergy product line: bicarbonato de sódio, bicarbonato de "
            "amônio, bicarbonato de potássio, sagu, fécula de batata, fécula "
            "de mandioca, polvilho doce and polvilho azedo. Keep the search "
            "tied to the selected Hermes Vendas / commercial context and do "
            "not use generic web search. For each destination country with an "
            "active SECOM record, draft a concise request asking for importers, "
            "buyers and any commercial insights that could increase sales "
            "probability. Queue the messages for approval before any real SMTP "
            "send. If there are no SECOM targets to contact, respond with "
            "[SILENT]."
        ),
        slots=[_DELIVER],
        skills=("hermes-mail-commercial-operations", "hermes-vendas"),
        tags=("sales", "secom", "reports"),
    ),
    AutomationBlueprint(
        key="important-mail",
