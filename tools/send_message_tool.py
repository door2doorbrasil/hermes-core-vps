...
SEND_MESSAGE_SCHEMA = {
    ...
    "description": (
        ...
        "For Telegram RFQ workflows, action='rfq_approval' renders a controlled approval card from ComprasDB and wires inline buttons back to the backend."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
                "type": "string",
                "enum": ["send", "list", "react", "unreact", "rfq_approval"],
            ...
            "rfq_batch_id": {
                "type": "integer",
                "description": "For action='rfq_approval': Compras RFQ batch id to render as an inline approval card."
            },
            "approval_notes": {
                "type": "string",
                "description": "Optional notes to include in the RFQ approval card or callback audit trail."
            }
        },
        "required": []
    }
...
def send_message_tool(args, **kw):
    ...
    if action == "rfq_approval":
        return _handle_rfq_approval(args)
...
def _handle_rfq_approval(args):
    """Render and send a Telegram RFQ approval card from ComprasDB state."""
    target = args.get("target", "")
    rfq_batch_id = args.get("rfq_batch_id")
    if not rfq_batch_id:
        return tool_error("'rfq_batch_id' is required when action='rfq_approval'")

    try:
        batch_id = int(rfq_batch_id)
    except (TypeError, ValueError):
        return tool_error("'rfq_batch_id' must be an integer")

    try:
        from gateway.config import Platform, load_gateway_config
        platform_name = (target.split(":", 1)[0] if target else "telegram").strip().lower()
        platform = Platform(platform_name)
    except (ValueError, KeyError):
        return tool_error(f"Unknown platform: {target or 'telegram'}")

    if platform_name != "telegram":
        return tool_error("RFQ approval cards are currently supported only on Telegram")

    try:
        from tools.compras_rfq_approval import load_rfq_batch_card_data
        batch_data, candidates = load_rfq_batch_card_data(batch_id)
    except Exception as exc:
        return json.dumps(_error(f"Failed to load RFQ batch {batch_id}: {exc}"))

    if not candidates:
        return tool_error(f"RFQ batch {batch_id} has no candidates to approve")

    try:
        from gateway.channel_directory import resolve_channel_name
        from gateway.run import _gateway_runner_ref
        from tools.compras_rfq_approval import button_rows_to_keyboard, render_rfq_approval_card
    except Exception as exc:
        return json.dumps(_error(f"RFQ approval helpers unavailable: {exc}"))

    runner = _gateway_runner_ref()
    if runner is None:
        return tool_error("RFQ approval cards require a live gateway with Telegram connected")

    adapter = runner.adapters.get(platform)
    if adapter is None or not hasattr(adapter, "send_rfq_approval_card"):
        return tool_error("Telegram adapter does not expose RFQ approval card support")
    ...
    if target and ":" in target:
        target_ref = target.split(":", 1)[1].strip()
        if target_ref:
            try:
                resolved = resolve_channel_name(platform_name, target_ref)
            except Exception:
                resolved = None
            chat_id, thread_id, _ = (None, None, False) if resolved is None else _parse_target_ref(platform_name, resolved)
            if chat_id is None:
                chat_id = resolved or target_ref
    ...
    if not chat_id:
        return tool_error("No Telegram target resolved for RFQ approval card")

    card = render_rfq_approval_card(batch_data, candidates)
    try:
        from model_tools import _run_async
        result = _run_async(
            adapter.send_rfq_approval_card(
                chat_id=str(chat_id),
                batch=batch_data,
                candidates=candidates,
                approval_notes=(args.get("approval_notes") or None),
                metadata={"thread_id": str(thread_id)} if thread_id else None,
            )
        )
    except Exception as exc:
        return json.dumps(_error(f"Failed to send RFQ approval card: {exc}"))
    ...
    return json.dumps({"success": bool(result), "batch_id": batch_id, "card": card})
