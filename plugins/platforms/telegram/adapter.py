...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def send_rfq_approval_card(
        self,
        chat_id: str,
        batch: Dict[str, Any],
        ...
        """Send an inline RFQ approval card with per-supplier buttons."""
        if not self._bot:
            return SendResult(success=False, error="Not connected")
        ...
        try:
            ...
            card = render_rfq_approval_card(batch, candidates)
            text = card["text"]
            if approval_notes:
                text += f"\n\n<b>Observações:</b> {_html.escape(str(approval_notes))}"
            keyboard = button_rows_to_keyboard(card["buttons"])
            ...
            logger.warning("[%s] send_rfq_approval_card failed: %s", self.name, e)
            return SendResult(success=False, error=str(e))
    ...
    async def _handle_callback_query(
        ...
        # --- RFQ approval callbacks (rfq:<batch_id>:<a|r>:<selector>) ---
        if data.startswith("rfq:"):
            ...
            action = parse_rfq_callback_data(data)
            if action is None:
                await query.answer(text="Invalid RFQ approval data.")
                return
            ...
            if not self._is_callback_user_authorized(
                ...
                await query.answer(text="⛔ You are not authorized to approve RFQs.")
                return

            await query.answer(text="⏳ Processando aprovação RFQ…")
            try:
                await query.edit_message_text(
                    text="⏳ Processando decisão RFQ…",
                    parse_mode=ParseMode.HTML,
                    reply_markup=None,
                )
            except Exception:
            ...
            try:
                ...
                logger.error("[%s] RFQ approval callback failed: %s", self.name, exc, exc_info=True)
                try:
                    await query.edit_message_text(
                        text=self.format_message(f"❌ RFQ approval failed: {exc}"),
                        parse_mode=ParseMode.MARKDOWN_V2,
                        reply_markup=None,
                    )
                except Exception:
