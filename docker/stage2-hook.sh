...
# Seed the curated Hermes Mail skeleton only when the target runtime tree
# has not been initialized yet. This keeps the VPS bootstrap reproducible
# without shipping historical mailbox data, backups, or local secrets.
if [ ! -d "$HERMES_HOME/hermes-mail/scripts" ] && [ -d "$INSTALL_DIR/data/hermes-mail/scripts" ]; then
    as_hermes mkdir -p \
        "$HERMES_HOME/hermes-mail" \
        "$HERMES_HOME/hermes-mail/state" \
        "$HERMES_HOME/hermes-mail/emails/incoming" \
        "$HERMES_HOME/hermes-mail/reports" \
        "$HERMES_HOME/hermes-mail/client-quotes"
    as_hermes cp -R "$INSTALL_DIR/data/hermes-mail/." "$HERMES_HOME/hermes-mail/"
fi
...
if [ -d "$HERMES_HOME/hermes-mail" ]; then
    chown -R hermes:hermes "$HERMES_HOME/hermes-mail" 2>/dev/null || true
fi
