...
async def get_sessions(
    ...
    from hermes_cli.dashboard_auth.session_context import get_current_session
    ...
    if not resolved_user_id:
        ...
        if current_session is not None:
            resolved_user_id = (current_session.user_id or "").strip()
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WHATSAPP_ALLOWED_USERS": {
        "description": "Comma-separated WhatsApp users allowed to message the number",
        "prompt": "WhatsApp message permissions",
    },
    "HASS_URL": {
...
def _ensure_profile_access(profile: Optional[str]) -> None:
    """Raise 403 when the current authenticated session cannot use *profile*."""
    requested = (profile or "").strip()
    if not requested or requested.lower() == "current" or requested == "default":
        return
    ...
    profile_dir = _resolve_profile_dir(requested)
    if not profiles_mod.profile_allows_session(profile_dir, session=session):
        raise HTTPException(
            status_code=403,
            detail=f"Not authorized for profile {requested!r}",
        )
...
@app.get("/api/profiles/selectable")
async def list_selectable_profiles_endpoint():
    from hermes_cli import profiles as profiles_mod
    from hermes_cli.dashboard_auth.session_context import get_current_session
    ...
    try:
        return {
            "profiles": [
                _profile_to_dict(p)
                for p in profiles_mod.list_accessible_profiles(session=session)
            ]
        }
    except Exception:
        _log.exception("GET /api/profiles/selectable failed")
        if session is None:
            return {"profiles": _fallback_profile_dicts(profiles_mod)}
        ...
        return {
            "profiles": [
                p
                for p in fallback
                if p.get("is_default") or profiles_mod.profile_allows_session(
                    profiles_mod.get_profile_dir(p["name"]),
                    session=session,
                )
            ]
        }
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    if explicit_source:
        _ensure_profile_access(explicit_source)
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    ...
    """
    from hermes_cli import profiles as profiles_mod
    _ensure_profile_access(body.name)
    try:
...
def _profile_scope(profile: Optional[str]):
    ...
    if not requested or requested.lower() == "current":
        ...
        _ensure_profile_access(requested)
        profile_dir = _resolve_profile_dir(requested)
        token = set_hermes_home_override(str(profile_dir))
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    auth_reason, cred, session_info = _ws_auth_reason(ws)
    mode = _ws_auth_mode()
    ...
    from hermes_cli.dashboard_auth.session_context import (
        reset_current_session,
        set_current_session,
    )
    session_token = set_current_session(session_info)
