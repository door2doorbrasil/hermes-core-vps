...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "nexosai":
        # nexos.ai exposes human-readable model ids that legitimately contain
        # spaces (e.g. "Claude Opus 4.8"); the /models ``id`` is the request
        # model param, so spaces are valid. Reject only tab/newline/control ws.
        _bad_whitespace = any(ch in "\t\n\r\f\v" for ch in requested)
    else:
        _bad_whitespace = any(ch.isspace() for ch in requested)
    if _bad_whitespace:
        return {
            "accepted": False,
            "persist": False,
