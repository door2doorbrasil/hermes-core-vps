...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    if not path.is_file():
        return {"description": "", "description_auto": False, "acl": None}
    try:
        import yaml
    ...
    return {
        ...
        "acl": data.get("acl") if isinstance(data.get("acl"), dict) else None,
    }
...
def _session_field(session: Any, field: str) -> str:
    if session is None:
        return ""
...
def _normalise_acl_values(values: Any) -> set[str]:
    if values is None:
        return set()
    ...
    for item in iterator:
        ...
        if text:
            normalised.add(text.casefold())
    return normalised


def _acl_value_set(acl: dict, *keys: str) -> set[str]:
    combined: set[str] = set()
    for key in keys:
        combined.update(_normalise_acl_values(acl.get(key)))
    return combined


def _session_groups(session: Any) -> set[str]:
    """Return a case-folded set of groups attached to *session*.

    Providers may surface groups explicitly via ``session.groups`` or,
    for backward compatibility, via a comma-separated ``org_id``.
    """
    if isinstance(session, dict):
        raw_groups = session.get("groups")
    else:
        raw_groups = getattr(session, "groups", ())
    groups = _normalise_acl_values(raw_groups)
    if groups:
        return groups
    org_id = _session_field(session, "org_id")
    if not org_id:
        return set()
    return {
        part.casefold()
        for part in org_id.split(",")
        if part.strip()
    }


def profile_allows_session(profile_dir: Path, session: Any = None) -> bool:
    """Return ``True`` when *session* may use the given profile.

    Backward-compatible default: profiles without an ``acl`` block remain
    usable by everyone. When an ACL is present, an authenticated session must
    match at least one allow-list entry unless ``public: true`` is set.

    Loopback mode passes ``session=None`` and is allowed through so the
    dashboard keeps behaving like the legacy local-only app. Gated mode must
    pass the verified session object; if it doesn't, the ACL check should not
    be used as a bypass.
    """
    ...
    if not isinstance(acl, dict):
        return True
    if bool(acl.get("public", False)):
        return True
    if session is None:
        return True

    allowed_groups = _acl_value_set(acl, "allowed_groups", "groups")
    allowed_user_ids = _acl_value_set(acl, "allowed_user_ids", "user_ids", "users")
    allowed_emails = _acl_value_set(acl, "allowed_emails", "emails", "email_addresses")
    allowed_org_ids = _acl_value_set(acl, "allowed_org_ids", "org_ids", "orgs")
    allowed_providers = _acl_value_set(acl, "allowed_providers", "providers")

    if not any(
        (allowed_groups, allowed_user_ids, allowed_emails, allowed_org_ids, allowed_providers)
    ):
        return False

    user_id = _session_field(session, "user_id").casefold()
    email = _session_field(session, "email").casefold()
    org_id = _session_field(session, "org_id").casefold()
    provider = _session_field(session, "provider").casefold()
    groups = _session_groups(session)

    return (
        (bool(groups & allowed_groups))
        or (user_id and user_id in allowed_user_ids)
        or (email and email in allowed_emails)
        or (org_id and org_id in allowed_org_ids)
        or (provider and provider in allowed_providers)
    )


def list_accessible_profiles(session: Any = None) -> List["ProfileInfo"]:
    """Return profiles the current session may use.

    The default profile is always included. Named profiles are filtered by
    :func:`profile_allows_session` only when a session object is present.
    """
    profiles = list_profiles()
    if session is None:
        return profiles

    allowed: List[ProfileInfo] = []
    for info in profiles:
        if info.is_default or profile_allows_session(info.path, session=session):
            allowed.append(info)
    return allowed
