...
class Session:
    ...
    All fields except ``groups`` are mandatory. Providers that don't have a
    concept of orgs should set ``org_id`` to an empty string.
    ``access_token`` and ``refresh_token`` are opaque to Hermes —
    provider-specific. ``groups`` is the preferred way to surface ACL
    membership and may be empty when the provider only exposes a legacy
    ``org_id`` string.
    """
