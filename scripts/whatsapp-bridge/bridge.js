...
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'bridge'); // "bot" or "bridge"
const WHATSAPP_ALLOW_ALL_USERS = ['1', 'true', 'yes', 'on'].includes(
  String(process.env.WHATSAPP_ALLOW_ALL_USERS || '').trim().toLowerCase(),
);
const ALLOWED_USERS = WHATSAPP_ALLOW_ALL_USERS
  ? new Set(['*'])
  : parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
...
function formatOutgoingMessage(message) {
  // Keep customer-facing replies clean: no auto-stamp, footer, or agent label.
  return message;
}
...
async function startSocket() {
  ...
  // In bridge mode, your own messages commonly arrive as 'append' rather
    ...
    for (const msg of messages) {
      ...
      // Bridge mode accepts inbound DMs from anyone; bot mode uses allowlists.
      if (!msg.key.fromMe) {
        if (ALLOWED_USERS.size > 0 && !matchesAllowedUser(senderId, ALLOWED_USERS, SESSION_DIR)) {
          try {
            console.log(JSON.stringify({
              event: 'ignored',
      ...
      // Ignore Hermes' own reply messages in legacy mode to avoid loops.
...
// HERMES_STRIP_CUSTOMER_VISIBLE_TOOL_TRACES
function stripCustomerVisibleToolTracesValue(value) {
  if (typeof value === "string") {
    return value
      ...
      .filter((line) => {
        ...
        // Nunca enviar rastros internos de ferramenta/skill para cliente
        if (/^📚\s*skill_view\s*:/i.test(s)) return false;
        if (/^skill_view\s*:/i.test(s)) return false;
        if (/^📚\s*skill\s+/i.test(s)) return false;
        if (/^skill view\(/i.test(s)) return false;
        if (/^tool calls?\b/i.test(s)) return false;
        if (/^└─\s*▾\s*tool calls?/i.test(s)) return false;
        if (/^├─\s*●\s*skill view/i.test(s)) return false;
        if (/^├─\s*●\s*search files/i.test(s)) return false;
        if (/^├─\s*●\s*read file/i.test(s)) return false;
        if (/^├─\s*●\s*terminal/i.test(s)) return false;
        if (/^┊\s*📚\s*skill/i.test(s)) return false;
  ...
  if (Array.isArray(value)) {
    return value.map(stripCustomerVisibleToolTracesValue);
  }

  if (value && typeof value === "object") {
    for (const key of Object.keys(value)) {
      value[key] = stripCustomerVisibleToolTracesValue(value[key]);
    }
    return value;
  }
...
function stripCustomerVisibleToolTracesMiddleware(req, res, next) {
  if (req && req.body) {
    req.body = stripCustomerVisibleToolTracesValue(req.body);
  }
  next();
}
// /HERMES_STRIP_CUSTOMER_VISIBLE_TOOL_TRACE
...
if (PAIR_ONLY) {
  ...
  app.listen(PORT, '127.0.0.1', () => {
    ...
    if (ALLOWED_USERS.size > 0) {
      ...
      console.log(`🔒 Bridge mode — any inbound DM can reach Hermes unless the gateway filters it.`);
      console.log(`   Set WHATSAPP_ALLOWED_USERS=<phone> to restrict access, or * for explicit open bot.`);
    }
    console.log();
    startSocket();
  });
