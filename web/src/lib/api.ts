...
export const api = {
  ...
  getComprasRfqDashboard: (limit = 12, offset = 0) =>
    fetchJSON<ComprasRfqDashboardResponse>(
      `/api/hermes/compras/rfq/dashboard?limit=${limit}&offset=${offset}`,
    ),
  getComprasRfqWorkerStatus: () =>
    fetchJSON<Record<string, unknown>>("/api/hermes/compras/rfq/worker/status"),
  runComprasRfqWorkerOnce: () =>
    fetchJSON<Record<string, unknown>>("/api/hermes/compras/rfq/worker/run-once", {
      method: "POST",
    }),
  runComprasRfqRealSourcing: (payload: Record<string, unknown>) =>
    fetchJSON<ComprasRfqRealSourcingResponse>("/api/hermes/compras/rfq/sourcing", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }),
  prepareComprasRfq: (payload: Record<string, unknown>) =>
    fetchJSON<Record<string, unknown>>("/api/hermes/compras/rfq/prepare", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }),
  approveComprasRfq: (payload: Record<string, unknown>) =>
    fetchJSON<Record<string, unknown>>("/api/hermes/compras/rfq/approve", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }),
  recordComprasInboundEmail: (payload: Record<string, unknown>) =>
    fetchJSON<Record<string, unknown>>("/api/hermes/compras/rfq/inbound", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }),
  recordComprasQuote: (payload: Record<string, unknown>) =>
    fetchJSON<Record<string, unknown>>("/api/hermes/compras/quotes/record", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }),
  calculateComprasSalePrice: (payload: Record<string, unknown>) =>
    fetchJSON<Record<string, unknown>>("/api/hermes/compras/pricing/calculate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }),
  buildComprasProposal: (payload: Record<string, unknown>) =>
    fetchJSON<Record<string, unknown>>("/api/hermes/compras/proposals/build", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }),
  getComprasProducts: (params?: { search?: string; limit?: number; offset?: number }) => {
    const search = params?.search ? `&search=${encodeURIComponent(params.search)}` : "";
    const limit = params?.limit ?? 100;
    const offset = params?.offset ?? 0;
    return fetchJSON<ComprasProductListResponse>(
      `/api/hermes/compras/products?limit=${limit}&offset=${offset}${search}`,
    );
  },
  upsertComprasProduct: (body: {
    name: string;
    ...
    fetchJSON<{ object: string; product: ComprasProductRecord }>(
      "/api/hermes/compras/products",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  getComprasSecoms: (activeOnly = true) =>
    fetchJSON<ComprasSecomListResponse>(
      `/api/hermes/compras/secoms?active_only=${activeOnly ? "1" : "0"}`,
    ),
  upsertComprasSecom: (body: {
    country: string;
    office_name: string;
    city?: string | null;
    ...
    fetchJSON<{ object: string; office: ComprasSecomOffice }>(
      "/api/hermes/compras/secoms",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
  ...
  recommendComprasSecoms: (params: {
    ...
    if (params.destination_country) {
      query.set("destination_country", params.destination_country);
    }
    ...
    return fetchJSON<{
      ...
      offices: ComprasSecomOffice[];
      limit: number;
    }>(`/api/hermes/compras/secoms/recommendations?${query.toString()}`);
  },
  /**
  ...
  getSessions: (
    ...
    fetchJSON<PaginatedSessions>(
      appendProfileParam(
        `/api/sessions?limit=${limit}&offset=${offset}&order=${order}${user_id ? `&user_id=${encodeURIComponent(user_id)}` : ""}`,
        profile,
      ),
    ),
  ...
  getSelectableProfiles: () =>
    fetchJSON<{ profiles: ProfileInfo[] }>("/api/profiles/selectable"),
  getActiveProfile: () =>
  ...
  // Session search (FTS5)
  searchSessions: (q: string, profile = getManagementProfile(), user_id?: string) =>
    fetchJSON<SessionSearchResponse>(
      appendProfileParam(
        `/api/sessions/search?q=${encodeURIComponent(q)}${user_id ? `&user_id=${encodeURIComponent(user_id)}` : ""}`,
        profile,
      ),
    ),
 ...
 * see Contract Anchor C4 in the plan). ``groups`` exposes the ACL groups
 * associated with the session when the provider has them; the AuthWidget
...
export interface ComprasRfqDashboardBatch {
...
export interface ComprasRfqDashboardResponse {
  object: string;
  summary: Record<string, number>;
  recent_batches: ComprasRfqDashboardBatch[];
  worker_status: Record<string, unknown>;
  limit: number;
  offset: number;
}

export interface ComprasRfqRealSourcingResponse {
  object: string;
  product_id: number;
  rfq_batch_id: number;
  ...
  candidate_count: number;
  candidates: Record<string, unknown>[];
  next_action_requires_user_authorization: boolean;
  status: string;
  audit_log_id: number;
  sourcing_stdout?: string;
}

export interface ComprasProductSalesBrief {
  product_family: string;
  mandatory_specs: string[];
  market_differentiators: string[];
  transactional_angles: string[];
...
export interface ComprasProductRecord {
  id: number;
  name: string;
  description: string | null;
  technical_specs: string | null;
  ...
  created_at: number;
  updated_at: number;
  sales_brief?: ComprasProductSalesBrief;
}

export interface ComprasProductListResponse {
  object: string;
  products: ComprasProductRecord[];
  total: number;
  limit: number;
  offset: number;
}

export interface ComprasSecomOffice {
  id: number;
  country: string;
  office_name: string;
  city: string | null;
  language: string | null;
  email_primary: string | null;
  email_alternatives: string[] | string | null;
  phone: string | null;
  website: string | null;
  commercial_sector: string | null;
  product_focus: string[] | string | null;
  active: number;
  followup_interval_days: number;
  report_interval_days: number;
  last_request_at: number | null;
...
export interface ComprasSecomListResponse {
  object: string;
  offices: ComprasSecomOffice[];
  countries: string[];
  total: number;
