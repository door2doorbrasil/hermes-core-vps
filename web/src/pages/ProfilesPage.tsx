      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {profiles.map((p) => {
            ...
            const isExpanded = !!expandedProfiles[p.name];
            return (
                  ...
                  {isRenaming ? (
                    ...
                    <>
                      ...
                      <div className="pt-2">
                        <button
                          type="button"
                          className="flex w-full items-center justify-between gap-2 border-t border-border/60 pt-2 text-left text-xs text-muted-foreground hover:text-foreground"
                        ...
                        >
                          <span className="font-medium uppercase tracking-wide">
                            {isExpanded ? "Collapse agent data" : "Expand agent data"}
                          </span>
                          <ChevronDown
                            className={cn(
                          ...
                          <div className="mt-3 rounded-md border border-border/70 bg-muted/20 p-3">
                            <div className="grid gap-2 text-xs sm:grid-cols-2">
                              ...
                              <div>
                                ...
                                <span className="block text-foreground">
                                  {p.distribution_name
                                    ? `${p.distribution_name}${p.distribution_version ? ` @ ${p.distribution_version}` : ""}`
                                    : "Not set"}
                                </span>
                              </div>
                              <div>
                                ...
                                <span className="block text-foreground">
                                  {[
                                    p.is_default ? "default" : null,
                                    p.has_env ? "env" : null,
                                    p.has_alias ? "alias" : null,
                                    p.description_auto ? "auto-description" : null,
                                  ]
                                    .filter(Boolean)
                                    .join(" / ") || "none"}
                                </span>
                            ...
                            {p.distribution_source && (
                              <div className="mt-2 text-xs text-muted-foreground">
                                Source: <span className="text-foreground">{p.distribution_source}</span>
                              </div>
                            )}
                            {p.description && (
                              <div className="mt-2 text-xs text-muted-foreground">
                                Description: <span className="text-foreground">{p.description}</span>
                              </div>
                            )}
                          </div>
                        )}
