...
import { ChevronDown, ChevronUp, Clock, Copy, Pause, Pencil, Play, Trash2, X, Zap } from "lucide-react";
...
export default function CronPage() {
  ...
  const [createProfileName, setCreateProfileName] = useState("default");
  ...
  const createProfile =
    createProfileName === "default" ? "default" : createProfileName;
  ...
  const openCreateModal = useCallback((profile?: string) => {
    setCreateProfileName(profile && profile !== "all" ? profile : selectedProfile === "all" ? "default" : selectedProfile);
    setPrompt("");
  ...
  const cloneJobToCreate = useCallback((job: CronJob) => {
    const profile = getJobProfile(job);
    const rawSchedule = getJobScheduleRaw(job);
    setCreateProfileName(profile);
    setPrompt(getJobPrompt(job));
    setScheduleState(
      rawSchedule
        ? { ...DEFAULT_SCHEDULE_STATE, mode: "custom", custom: rawSchedule }
        : { ...DEFAULT_SCHEDULE_STATE },
    );
    setName(getJobName(job));
    setDeliver(asText(job.deliver) || "local");
    setJobSkills(Array.isArray(job.skills) ? job.skills.filter(Boolean) : []);
    setCreateModalOpen(true);
  }, []);
  ...
  const toggleJobExpanded = useCallback((jobKey: string) => {
  ...
  }, [openCreateModal, setEnd, t.common.create, loading]);
    ...
    <div className="flex flex-col gap-6">
            ...
            <div className="p-5 grid gap-4">
              <div className="grid gap-2">
                ...
                <Select
                  id="cron-profile"
                  value={createProfile}
                  onValueChange={(v) => setCreateProfileName(v)}
                >
      ...
      <div className="flex flex-col gap-3">
        ...
        {jobs.map((job) => {
          ...
          const isExpanded = Boolean(expandedJobs[jobKey]);
          const rawSchedule = getJobScheduleRaw(job);
          ...
          return (
            <Card key={jobKey}>
              <CardContent className="flex flex-col gap-4 py-4">
                ...
                {isExpanded && (
                  <div className="grid gap-3 rounded-xl border border-border bg-background/50 p-4 text-xs text-muted-foreground">
                    <div className="grid gap-1">
                      <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">Prompt completo</span>
                      <pre className="whitespace-pre-wrap font-courier text-foreground">{promptText || "—"}</pre>
                    </div>
                    <div className="grid gap-2 sm:grid-cols-2">
                      <div className="grid gap-1">
                        <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">Schedule bruto</span>
                        <span className="font-mono-ui text-foreground">{rawSchedule || "—"}</span>
                      </div>
                      <div className="grid gap-1">
                        <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">Estado interno</span>
                        <span className="font-mono-ui text-foreground">{getJobState(job)}</span>
                      </div>
                      <div className="grid gap-1">
                        <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">Profile</span>
                        <span className="font-mono-ui text-foreground">{profileLabel(profile)}</span>
                      </div>
                      <div className="grid gap-1">
                        <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">Deliver</span>
                        <span className="font-mono-ui text-foreground">{deliver || "local"}</span>
                      </div>
                    </div>
                    <div className="grid gap-1">
                      <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">Skills</span>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      <div className="grid gap-1">
                        <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">Última execução</span>
                        <span className="font-mono-ui text-foreground">{formatTime(job.last_run_at)}</span>
                      </div>
                      <div className="grid gap-1">
                        <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">Próxima execução</span>
                        <span className="font-mono-ui text-foreground">{formatTime(job.next_run_at)}</span>
                      </div>
                    </div>
                    <div className="grid gap-1">
                      <span className="uppercase tracking-[0.18em] text-[10px] text-muted-foreground/80">JSON bruto</span>
                      <pre className="max-h-56 overflow-auto rounded-lg bg-slate-950/80 p-3 font-mono-ui text-[11px] text-slate-100">
