...
export default function SessionsPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((me) => {
        if (cancelled) return;
        setAuthMe(me);
        setUserDraft((current) => current || me.user_id);
        setUserFilter((current) => current || me.user_id);
      })
      .catch(() => {
  ...
  useEffect(() => {
    const loadOverview = () => {
      api
        ...
        .catch(() => {});
      api
        .getSessions(50, 0, undefined, "recent", userFilter.trim() || undefined)
        .then((r) => {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      api
        .searchSessions(search.trim(), undefined, userFilter.trim() || undefined)
  ...
  const userSuggestions = useMemo(() => {
    ...
    for (const session of [...sessions, ...overviewSessions]) {
      const id = (session.user_id ?? "").trim();
      if (id) ids.add(id);
    }
    if (authMe?.user_id) ids.add(authMe.user_id);
    return [...ids].sort();
  }, [authMe?.user_id, overviewSessions, sessions]);
  const selectedUserLabel =
    userFilter.trim() === ""
      ? "Todos os usuários"
      : userFilter.trim() === authMe?.user_id
        ? "Meu usuário"
        : userFilter.trim();
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3">
            ...
            {showList && (
              <div className="flex min-w-0 flex-1 flex-wrap items-end gap-2 sm:flex-none">
                <label className="flex min-w-[14rem] flex-1 flex-col gap-1 text-xs text-text-secondary sm:flex-none">
                  ...
                  <Input
                    ...
                    placeholder={authMe?.user_id || "Escolha um usuário"}
                    list="session-user-suggestions"
                    className="h-8 py-0 text-xs leading-none"
                  />
                </label>
                <Button
                  outlined
                ...
                <div className="rounded border border-border bg-muted/20 px-3 py-2 text-xs text-text-secondary">
                  <div className="text-text-tertiary">Filtro ativo</div>
                  <div className="mt-0.5 font-medium text-text-primary">
                    {selectedUserLabel}
                  </div>
                </div>
                <datalist id="session-user-suggestions">
                  {userSuggestions.map((userId: string) => (
