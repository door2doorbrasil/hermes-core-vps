...
export function ProfileProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    Promise.all([api.getSelectableProfiles(), api.getActiveProfile()])
      .then(([profilesRes, info]) => {
        if (cancelled) return;

        const selectable = profilesRes.profiles.map((p) => p.name);
        ...
        if (urlProfile && !selectableSet.has(urlProfile)) {
        ...
        if (!urlProfile && active !== current && selectableSet.has(active)) {
