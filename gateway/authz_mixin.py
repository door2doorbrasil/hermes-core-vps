        ...
        if not platform_allowlist and not group_user_allowlist and not group_chat_allowlist and not global_allowlist:
            ...
            if self._adapter_enforces_own_access_policy(source.platform):
                ...
                if effective_policy == "allowlist":
                    return True
                # WhatsApp is intentionally open to any sender when the
                # adapter itself is configured in open mode. That keeps the
                # gateway from forcing pairing codes on fresh phone-number
                # contacts after a phone resync.
                if (
                    source.platform in {Platform.WHATSAPP, Platform.WHATSAPP_CLOUD}
                    and source.chat_type == "dm"
                    and effective_policy in {"", "open"}
                ):
                    return True
