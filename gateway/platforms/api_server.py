...
from gateway.compras_routing import apply_compras_prompt_suffix, route_compras_turn
...
def _content_has_image(content: Any) -> bool:
    """Return True when a request payload contains any image part."""
    if isinstance(content, dict):
        ptype = str(content.get("type") or "").strip().lower()
        if ptype in _IMAGE_PART_TYPES:
            return True
        nested = content.get("content")
        return _content_has_image(nested)
    if isinstance(content, list):
        return any(_content_has_image(part) for part in content)
    return False
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        model = model_name or _resolve_gateway_model()
    ...
    async def _handle_compras_health(self, request: "web.Request") -> "web.Response":
        """GET /api/hermes/compras/health — report compras DB readiness."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        from hermes_compras_state import open_compras_db, get_compras_db_path
        with open_compras_db() as db:
            return web.json_response({
                "object": "hermes.compras.health",
                "db_path": str(get_compras_db_path()),
                "table_count": len(db.list_tables()),
                "ready": True,
            })

    async def _handle_compras_products(self, request: "web.Request") -> "web.Response":
        """GET/POST /api/hermes/compras/products — list or upsert product registry entries."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        from hermes_compras_state import open_compras_db
        if request.method == "GET":
            search = str(request.query.get("search") or "").strip() or None
            try:
                limit = max(1, min(int(request.query.get("limit", "100")), 250))
            except (TypeError, ValueError):
                limit = 100
            try:
                ...
                offset = 0
            with open_compras_db() as db:
                products = [row.data for row in db.list_products(search=search, limit=limit, offset=offset)]
                total = db.fetchone(
                    """
                    SELECT COUNT(*) AS c
                    FROM products
                    WHERE (? IS NULL OR name LIKE ? OR description LIKE ? OR application LIKE ? OR packaging LIKE ?)
                    """,
                    (
                        search,
            ...
            return web.json_response({
                "object": "hermes.compras.products",
                "products": products,
                "total": int(total.data.get("c") or 0) if total is not None else len(products),
                "limit": limit,
                "offset": offset,
            })
        ...
        if not name:
            return web.json_response(_openai_error("Missing 'name' field", code="missing_product_name"), status=400)
        now = time.time()
        with open_compras_db() as db:
            product_id = db.upsert_product(
                name=name,
                description=str(body.get("description") or "").strip() or None,
                technical_specs=json.dumps(body.get("technical_specs"), ensure_ascii=False) if body.get("technical_specs") is not None and not isinstance(body.get("technical_specs"), str) else (str(body.get("technical_specs")) if body.get("technical_specs") is not None else None),
                application=str(body.get("application") or "").strip() or None,
                unit=str(body.get("unit") or "").strip() or None,
                ncm=str(body.get("ncm") or "").strip() or None,
                hs_code=str(body.get("hs_code") or "").strip() or None,
                packaging=str(body.get("packaging") or "").strip() or None,
                created_at=now,
                updated_at=now,
            )
            row = db.get_product_by_id(product_id)
        product = row.data if row is not None else {"id": product_id, "name": name}
        if row is not None:
            product["sales_brief"] = db.build_product_sales_brief(product)
        return web.json_response({
            "object": "hermes.compras.product",
            "product": product,
        }, status=201)

    async def _handle_compras_secoms(self, request: "web.Request") -> "web.Response":
        """GET/POST /api/hermes/compras/secoms — inspect or upsert the SECOM registry."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        from hermes_compras_state import open_compras_db
        if request.method == "GET":
            try:
                active_only = str(request.query.get("active_only", "1")).strip().lower() not in {"0", "false", "no", "off"}
            except Exception:
                active_only = True
            with open_compras_db() as db:
                offices = [row.data for row in db.list_secom_offices(active_only=active_only)]
                countries = db.list_secom_countries(active_only=active_only)
            return web.json_response({
                "object": "hermes.compras.secoms",
                "offices": offices,
                "countries": countries,
                "total": len(offices),
        ...
        office_name = str(body.get("office_name") or body.get("embassy") or "").strip()
        if not country or not office_name:
            return web.json_response(_openai_error("Missing 'country' or 'office_name'", code="missing_secom_fields"), status=400)
        now = time.time()
        with open_compras_db() as db:
            office_id = db.upsert_secom_office(
                ...
                email_primary=str(body.get("email_primary") or "").strip() or None,
                email_alternatives=body.get("email_alternatives") or [],
                phone=str(body.get("phone") or "").strip() or None,
                website=str(body.get("website") or "").strip() or None,
                commercial_sector=str(body.get("commercial_sector") or "").strip() or None,
                product_focus=body.get("product_focus") or [],
                active=_coerce_request_bool(body.get("active"), True),
                followup_interval_days=int(body.get("followup_interval_days") or 45),
                report_interval_days=int(body.get("report_interval_days") or 45),
                last_request_at=float(body.get("last_request_at")) if body.get("last_request_at") is not None else None,
                last_report_at=float(body.get("last_report_at")) if body.get("last_report_at") is not None else None,
                source=str(body.get("source") or "manual").strip() or "manual",
                notes=str(body.get("notes") or "").strip() or None,
                created_at=now,
                updated_at=now,
            )
            office_row = db.fetchone("SELECT * FROM secom_offices WHERE id = ?", (office_id,))
        return web.json_response({
            "object": "hermes.compras.secom",
            "office": office_row.data if office_row is not None else {"id": office_id},
        }, status=201)

    async def _handle_compras_secom_recommendations(self, request: "web.Request") -> "web.Response":
        """GET /api/hermes/compras/secoms/recommendations — rank SECOM offices by product line."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        product_name = str(request.query.get("product") or request.query.get("product_name") or "").strip()
        if not product_name:
            return web.json_response(_openai_error("Missing 'product' query parameter", code="missing_product"), status=400)
        destination_country = str(request.query.get("destination_country") or "").strip() or None
        try:
            limit = max(1, min(int(request.query.get("limit", "8")), 25))
        except (TypeError, ValueError):
            limit = 8
        from hermes_compras_state import open_compras_db
        with open_compras_db() as db:
            offices = [row.data for row in db.recommend_secom_offices_for_product(
                product_name=product_name,
                destination_country=destination_country,
                limit=limit,
            )]
        return web.json_response({
            "object": "hermes.compras.secom.recommendations",
    ...
    async def _handle_compras_rfq_dashboard(self, request: "web.Request") -> "web.Response":
        """GET /api/hermes/compras/rfq/dashboard — summarize recent RFQ activity."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        try:
        ...
        status_path = Path(__file__).resolve().parents[2] / "data" / "hermes-mail" / "state" / "rfq-email-worker.json"
        worker_status: dict[str, Any] = {"state": "unknown", "status_path": str(status_path), "present": False}
        if status_path.exists():
            try:
                loaded = json.loads(status_path.read_text(encoding="utf-8"))
                if isinstance(loaded, dict):
                    worker_status = dict(loaded)
                    worker_status.setdefault("state", "unknown")
                    worker_status["status_path"] = str(status_path)
                    worker_status["present"] = True
                else:
                    worker_status = {
                        "state": "error",
                        "status_path": str(status_path),
                        "present": True,
                        "error": "worker status file is not a JSON object",
                    }
        ...
        with open_compras_db() as db:
            batches = [row.data for row in db.list_rfq_batches(limit=limit, offset=offset)]
            batch_ids = [int(batch.get("id") or 0) for batch in batches]
            candidate_rows = {batch_id: [row.data for row in db.list_rfq_candidates(batch_id)] for batch_id in batch_ids}
            candidate_counts = {batch_id: len(db.list_rfq_candidates(batch_id)) for batch_id in batch_ids}
            quote_rows = db.list_supplier_quotes()
            summary = db.fetchone(
                """
                SELECT
                    COUNT(*) AS batches,
                    COALESCE((SELECT COUNT(*) FROM rfq_candidates), 0) AS candidates,
                    COALESCE((SELECT COUNT(*) FROM rfq_recipients), 0) AS recipients,
                    COALESCE((SELECT COUNT(*) FROM supplier_quotes), 0) AS quotes,
                    COALESCE((SELECT COUNT(*) FROM rfq_inbound_emails), 0) AS inbound_emails,
                    COALESCE((SELECT COUNT(*) FROM rfq_email_logs), 0) AS email_logs
                FROM rfq_batches
                """
            )
        ...
        for batch in batches:
            ...
            recent_batches.append({
                "batch": batch,
                "candidate_count": candidate_counts.get(batch_id, 0),
                "quote_count": int(batch.get("quote_count") or 0),
                "recipient_count": int(batch.get("recipient_count") or 0),
                "inbound_count": int(batch.get("inbound_count") or 0),
                "email_log_count": int(batch.get("email_log_count") or 0),
                "latest_quote": next((quote for quote in quote_rows if int(quote.data.get("rfq_batch_id") or 0) == batch_id), None),
                "candidates": candidate_rows.get(batch_id, []),
            })

        return web.json_response({
            "object": "hermes.compras.rfq.dashboard",
            "summary": summary.data if summary is not None else {},
    ...
    async def _handle_compras_rfq_worker_status(self, request: "web.Request") -> "web.Response":
        """GET /api/hermes/compras/rfq/worker/status — read the worker heartbeat."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        status_path = Path(__file__).resolve().parents[2] / "data" / "hermes-mail" / "state" / "rfq-email-worker.json"
    ...
    async def _handle_compras_rfq_worker_run_once(self, request: "web.Request") -> "web.Response":
        """POST /api/hermes/compras/rfq/worker/run-once — execute one worker sweep."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        scripts_dir = Path(__file__).resolve().parents[2] / "data" / "hermes-mail" / "scripts"
        if str(scripts_dir) not in sys.path:
            sys.path.insert(0, str(scripts_dir))
        from rfq_email_worker import run_worker_cycle

        result = run_worker_cycle()
        return web.json_response({
            "object": "hermes.compras.rfq.worker.run-once",
            "result": result,
        }, status=201)

    def _resolve_hermes_mail_root(self) -> Path:
        """Return the hermes-mail data root for the current checkout."""
        local_root = Path(__file__).resolve().parents[2] / "data" / "hermes-mail"
    ...
    def _run_real_compras_sourcing(self, body: dict[str, Any]) -> dict[str, Any]:
        """Run the real sourcing workflow and mirror the output into Compras."""
        from hermes_compras_state import open_compras_db

        product_name = str(body.get("product") or body.get("product_name") or "").strip()
        if not product_name:
            raise ValueError("Missing 'product' field")

        mail_root = self._resolve_hermes_mail_root()
        scripts_dir = mail_root / "scripts"
        sourcing_script = scripts_dir / "sourcing_research.py"
        if not sourcing_script.exists():
            raise FileNotFoundError(f"Missing sourcing script: {sourcing_script}")
        ...
        command = [
            ...
            product_name,
            "--region",
            str(body.get("region") or "global"),
            "--country",
            str(body.get("country") or "global"),
        ]
        ...
        for line in completed.stdout.splitlines():
            if line.startswith("project_id="):
                project_id = line.split("=", 1)[1].strip() or None
                break

        qualification_path = mail_root / "manufacturer-qualification.jsonl"
        source_records = self._load_jsonl_records(qualification_path)
        if project_id:
            source_records = [record for record in source_records if str(record.get("project_id") or "") == project_id]
        source_records.sort(
            key=lambda record: (
                0 if record.get("approved_for_rfq") else 1,
                -float(record.get("final_score") or 0.0),
                -float(record.get("manufacturer_score") or 0.0),
                str(record.get("company_name") or "").lower(),
            )
        )
        ...
        with open_compras_db() as db:
            ...
            batch_id = db.insert_rfq_batch(
                batch_code=str(body.get("batch_code") or f"RFQ-REAL-{int(now)}"),
                product_id=product_id,
                requested_by=str(body.get("requested_by") or body.get("user") or "").strip() or None,
            ...
            for record in source_records[:20]:
                ...
                company_name = str(
                    ...
                    or "Fornecedor qualificado"
                ).strip()
                if not company_name:
                    continue
                legal_name = str(profile.get("legal_name") or company_name).strip()
                trade_name = str(profile.get("trade_name") or company_name).strip()
                website = str(record.get("website") or profile.get("website") or "").strip() or None
                source_urls_value = record.get("source_urls")
                first_source_url = source_urls_value[0] if isinstance(source_urls_value, list) and source_urls_value else None
                source_url = str(first_source_url or record.get("query_url") or website or "").strip() or None
                company_type = str(profile.get("company_type") or "").strip().lower()
                source_tier = str(record.get("source_tier") or record.get("discovery_channel") or "").strip().lower()
                manufacturer_flag = bool(
                    record.get("approved_for_rfq")
                    or "manufacturer" in company_type
                    or source_tier in {"official_site", "manufacturer_catalog", "export_registry"}
                )
                trading_company_flag = bool(
                    "trading" in company_type
                    or source_tier in {"marketplace-b2b", "trading_platform"}
                )
                supplier_payload = {
                    "legal_name": legal_name,
                    "trade_name": trade_name,
                    "supplier_type": profile.get("company_type") or ("manufacturer" if manufacturer_flag else "unknown"),
                    "country": profile.get("country") or record.get("country") or body.get("country") or None,
                    "city": profile.get("city") or record.get("city") or body.get("region") or None,
                    "website": website,
                    "source_url": source_url,
                    "source_name": "sourcing_research",
                    "source_type": source_tier or "official_site",
                    "manufacturer_flag": manufacturer_flag,
                    "trading_company_flag": trading_company_flag,
                    "verified_status": "approved_for_rfq" if record.get("approved_for_rfq") else "partially_verified" if record.get("manual_review_required") else "unverified",
                    "data_quality_status": "ready_for_rfq" if record.get("approved_for_rfq") else "needs_review" if record.get("manual_review_required") else "pending_validation",
                    "notes": str(
                        record.get("qualification_summary")
                        or record.get("notes")
                        or f"final_score={record.get('final_score')}"
                    ),
                    "general_email": profile.get("email") or None,
                    "sales_email": profile.get("email") or None,
                    "phone": profile.get("telephone") or None,
                    "whatsapp": profile.get("whatsapp") or None,
                    "wechat": profile.get("wechat") or None,
                    "contact_page_url": profile.get("contact_page_url") or None,
                    "product_focus": profile.get("products") or [],
                ...
                existing_supplier_id = db._find_existing_supplier_id(supplier_payload)
                if existing_supplier_id is None:
                    supplier_id = db.insert_supplier_candidate(
                        legal_name=legal_name,
                        trade_name=trade_name,
                        supplier_type=str(supplier_payload["supplier_type"]),
                        country=supplier_payload.get("country"),
                        city=supplier_payload.get("city"),
                        website=website,
                        source_url=source_url,
                        source_name="sourcing_research",
                        source_type=supplier_payload["source_type"],
                        manufacturer_flag=manufacturer_flag,
                        trading_company_flag=trading_company_flag,
                        verified_status=str(supplier_payload["verified_status"]),
                        data_quality_status=str(supplier_payload["data_quality_status"]),
                        notes=str(supplier_payload["notes"]),
                        created_at=now,
                        updated_at=now,
                    )
                else:
                ...
                contact_id = None
                if any([profile.get("contact_person"), profile.get("email"), profile.get("telephone"), profile.get("whatsapp"), profile.get("wechat")]):
                    contact_id = db.upsert_supplier_contact(
                        supplier_id=supplier_id,
                        name=str(profile.get("contact_person") or profile.get("contact_name") or "").strip() or None,
                        role=str(profile.get("contact_title") or "").strip() or None,
                        department=str(profile.get("department") or "").strip() or None,
                        email=str(profile.get("email") or "").strip() or None,
                        phone=str(profile.get("telephone") or "").strip() or None,
                        whatsapp=str(profile.get("whatsapp") or "").strip() or None,
                        wechat=str(profile.get("wechat") or "").strip() or None,
                        linkedin=str(profile.get("linkedin") or "").strip() or None,
                        language=str(profile.get("language") or "").strip() or None,
                        is_primary=True,
                        source="sourcing_research",
                        source_email_id=str(project_id or ""),
                        source_attachment_id=str(record.get("id") or ""),
                        verified_status="verified" if profile.get("email") else "unverified",
                        created_at=now,
                        updated_at=now,
                    )
                supplier_payload["supplier_id"] = supplier_id
                if contact_id is not None:
                    supplier_payload["contact_id"] = contact_id
                candidate_suppliers.append(supplier_payload)

            if not candidate_suppliers:
                fallback_candidates = body.get("candidate_suppliers") or []
                if isinstance(fallback_candidates, list):
                    candidate_suppliers = [candidate for candidate in fallback_candidates if isinstance(candidate, dict)]

            candidate_ids = db.store_rfq_candidates(batch_id, candidate_suppliers, created_at=now, updated_at=now)
            stored_candidates = [row.data for row in db.list_rfq_candidates(batch_id)]
            audit_id = db.insert_audit_log(
                action="rfq_real_sourcing_run",
                actor=str(body.get("requested_by") or body.get("user") or "") or None,
        ...
        return {
            "object": "hermes.compras.rfq.real_sourcing",
            "product_id": product_id,
            ...
            "sourcing_stdout": completed.stdout,
        }

    async def _handle_compras_prepare_rfq(self, request: "web.Request") -> "web.Response":
        """POST /api/hermes/compras/rfq/prepare — create a draft RFQ batch."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        body, err = await self._read_json_body(request)
        ...
        product_name = str(body.get("product") or body.get("product_name") or "").strip()
        if not product_name:
            return web.json_response(_openai_error("Missing 'product' field", code="missing_product"), status=400)
        candidate_suppliers = body.get("candidate_suppliers") or []
        if not isinstance(candidate_suppliers, list):
            return web.json_response(_openai_error("'candidate_suppliers' must be an array", code="invalid_candidate_suppliers"), status=400)
        requested_by = str(body.get("requested_by") or body.get("user") or "").strip() or None
        batch_code = str(body.get("batch_code") or f"RFQ-{int(time.time())}-{uuid.uuid4().hex[:6]}").strip()
        now = time.time()
        with open_compras_db() as db:
            product_id = db.insert_product(
                ...
                updated_at=now,
                description=str(body.get("description") or "").strip() or None,
                technical_specs=json.dumps(body.get("technical_specs") or {}, ensure_ascii=False) if body.get("technical_specs") is not None else None,
                application=str(body.get("application") or "").strip() or None,
                unit=str(body.get("unit") or "").strip() or None,
                ncm=str(body.get("ncm") or "").strip() or None,
                hs_code=str(body.get("hs_code") or "").strip() or None,
            ...
            stored_candidates = [row.data for row in db.list_rfq_candidates(batch_id)]
            audit_id = db.insert_audit_log(
                action="rfq_candidate_suppliers_presented",
                actor=requested_by,
                entity_type="rfq_batch",
                entity_id=str(batch_id),
                payload_json=json.dumps({
                    "product": product_name,
                    "candidate_suppliers": stored_candidates,
                    "next_action_requires_user_authorization": True,
                }, ensure_ascii=False),
                created_at=now,
            )
        return web.json_response({
            "object": "hermes.compras.rfq.prepare",
            "batch_code": batch_code,
            "rfq_batch_id": batch_id,
            "product_id": product_id,
            "product": product_name,
    ...
    async def _handle_compras_rfq_real_sourcing(self, request: "web.Request") -> "web.Response":
        """POST /api/hermes/compras/rfq/sourcing — run the real sourcing workflow."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        body, err = await self._read_json_body(request)
        if err:
            return err
        try:
            result = self._run_real_compras_sourcing(body)
        except ValueError as exc:
            return web.json_response(_openai_error(str(exc), code="missing_product"), status=400)
        except FileNotFoundError as exc:
            return web.json_response(_openai_error(str(exc), code="sourcing_script_missing"), status=500)
        except RuntimeError as exc:
            return web.json_response(_openai_error(str(exc), code="sourcing_run_failed"), status=502)
        return web.json_response(result, status=201)

    async def _handle_compras_approve_rfq(self, request: "web.Request") -> "web.Response":
        """POST /api/hermes/compras/rfq/approve — approve or reject candidates."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        body, err = await self._read_json_body(request)
        if err:
            return err
        try:
            rfq_batch_id = int(body.get("rfq_batch_id"))
        except (TypeError, ValueError):
            return web.json_response(_openai_error("Missing or invalid 'rfq_batch_id'", code="invalid_rfq_batch_id"), status=400)
        approved = body.get("approved_supplier_candidates") or []
        rejected = body.get("rejected_supplier_candidates") or []
        if not isinstance(approved, list) or not isinstance(rejected, list):
            return web.json_response(_openai_error("'approved_supplier_candidates' and 'rejected_supplier_candidates' must be arrays", code="invalid_candidates"), status=400)
        approved_by = str(body.get("approved_by") or body.get("user_id") or "").strip() or None
        approval_notes = body.get("approval_notes")
        authorize_email_send = bool(body.get("authorize_email_send", False))
        dry_run_value = os.environ.get("DRY_RUN", "true").strip().lower()
        dry_run = dry_run_value not in {"0", "false", "no", "off"}
        now = time.time()
        with open_compras_db() as db:
            result = db.approve_rfq_suppliers(
                rfq_batch_id=rfq_batch_id,
                approved_supplier_candidates=approved,
                rejected_supplier_candidates=rejected,
        ...
        result.update({
            "object": "hermes.compras.rfq.approve",
            "next_action": result.get("next_action") or ("send_pending_email_logs" if authorize_email_send else "no_email_send"),
        })
        return web.json_response(result)

    async def _handle_compras_record_inbound_email(self, request: "web.Request") -> "web.Response":
        """POST /api/hermes/compras/rfq/inbound — persist a supplier reply."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        body, err = await self._read_json_body(request)
        ...
        try:
            rfq_batch_id = int(body.get("rfq_batch_id")) if body.get("rfq_batch_id") is not None else None
        except (TypeError, ValueError):
            return web.json_response(_openai_error("Missing or invalid 'rfq_batch_id'", code="invalid_rfq_batch_id"), status=400)
        try:
            supplier_id = int(body.get("supplier_id")) if body.get("supplier_id") is not None else None
        except (TypeError, ValueError):
            return web.json_response(_openai_error("Missing or invalid 'supplier_id'", code="invalid_supplier_id"), status=400)
        try:
            contact_id = int(body.get("contact_id")) if body.get("contact_id") is not None else None
        except (TypeError, ValueError):
            return web.json_response(_openai_error("Missing or invalid 'contact_id'", code="invalid_contact_id"), status=400)

        from hermes_compras_state import open_compras_db
        with open_compras_db() as db:
            if supplier_id is not None and contact_id is None and any(
                str(body.get(field) or "").strip()
                for field in ("contact_name", "contact_email", "contact_phone", "contact_whatsapp")
            ):
                contact_id = db.upsert_supplier_contact(
                    supplier_id=supplier_id,
                    name=str(body.get("contact_name") or "").strip() or None,
                    role=str(body.get("contact_role") or "").strip() or None,
                    department=str(body.get("contact_department") or "").strip() or None,
                    email=str(body.get("contact_email") or "").strip() or None,
                    phone=str(body.get("contact_phone") or "").strip() or None,
                    whatsapp=str(body.get("contact_whatsapp") or "").strip() or None,
                    wechat=str(body.get("contact_wechat") or "").strip() or None,
                    linkedin=str(body.get("contact_linkedin") or "").strip() or None,
                    language=str(body.get("contact_language") or "").strip() or None,
                    is_primary=_coerce_request_bool(body.get("contact_is_primary"), False),
                    source=str(body.get("contact_source") or "rfq_inbound_email").strip(),
                    source_email_id=str(body.get("message_id") or "").strip() or None,
                    source_attachment_id=str(body.get("raw_payload_path") or "").strip() or None,
                    verified_status=str(body.get("contact_verified_status") or "unverified").strip(),
            ...
            inbound_email_id = db.record_rfq_inbound_email(
                ...
                email_references=str(body.get("email_references") or "").strip() or None,
                from_email=str(body.get("from_email") or "").strip() or None,
                from_name=str(body.get("from_name") or "").strip() or None,
                to_email=str(body.get("to_email") or "").strip() or None,
                cc=str(body.get("cc") or "").strip() or None,
                subject=str(body.get("subject") or "").strip() or None,
                received_at=received_at,
                body_text=str(body.get("body_text") or "").strip() or None,
                body_html=str(body.get("body_html") or "").strip() or None,
                body_summary=str(body.get("body_summary") or "").strip() or None,
                detected_language=str(body.get("detected_language") or "").strip() or None,
                has_attachments=_coerce_request_bool(body.get("has_attachments"), False),
                attachment_count=int(body.get("attachment_count") or 0),
                raw_payload_path=str(body.get("raw_payload_path") or "").strip() or None,
                processing_status=str(body.get("processing_status") or "received").strip(),
                extraction_status=str(body.get("extraction_status") or "pending").strip(),
                linked_outbound_email_log_id=int(body.get("linked_outbound_email_log_id")) if body.get("linked_outbound_email_log_id") is not None else None,
                provider_thread_id=str(body.get("provider_thread_id") or "").strip() or None,
                email_thread_id=str(body.get("email_thread_id") or "").strip() or None,
                correlation_token=str(body.get("correlation_token") or "").strip() or None,
                matched_by=str(body.get("matched_by") or "").strip() or None,
                matching_confidence=float(body.get("matching_confidence")) if body.get("matching_confidence") is not None else None,
                is_direct_reply=bool(body.get("is_direct_reply", False)),
                is_followup_reply=bool(body.get("is_followup_reply", False)),
                created_at=received_at,
                updated_at=received_at,
            )

        return web.json_response({
            "object": "hermes.compras.rfq.inbound",
            "rfq_batch_id": rfq_batch_id,
            "supplier_id": supplier_id,
            "contact_id": contact_id,
    ...
    async def _handle_compras_record_supplier_quote(self, request: "web.Request") -> "web.Response":
        """POST /api/hermes/compras/quotes/record — persist a supplier quote."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        body, err = await self._read_json_body(request)
        ...
        try:
            ...
            return web.json_response(_openai_error("Missing or invalid 'rfq_batch_id' or 'supplier_id'", code="invalid_quote_refs"), status=400)
        if rfq_batch_id is None or supplier_id is None:
            return web.json_response(_openai_error("'rfq_batch_id' and 'supplier_id' are required", code="missing_quote_refs"), status=400)

        created_at = time.time()
        from hermes_compras_state import open_compras_db
        with open_compras_db() as db:
            contact_id = int(body.get("contact_id")) if body.get("contact_id") is not None else None
            if contact_id is None and any(
                str(body.get(field) or "").strip()
                for field in ("contact_name", "contact_email", "contact_phone", "contact_whatsapp")
            ):
                contact_id = db.upsert_supplier_contact(
                    ...
                    language=str(body.get("contact_language") or "").strip() or None,
                    is_primary=_coerce_request_bool(body.get("contact_is_primary"), False),
                    source=str(body.get("contact_source") or "supplier_quote").strip(),
                    source_email_id=str(body.get("message_id") or "").strip() or None,
                    source_attachment_id=str(body.get("raw_payload_path") or "").strip() or None,
                    verified_status=str(body.get("contact_verified_status") or "unverified").strip(),
            ...
            quote_id = db.upsert_supplier_quote(
                ...
                supplier_id=supplier_id,
                contact_id=contact_id,
                product_id=int(body.get("product_id")) if body.get("product_id") is not None else None,
                source_type=str(body.get("source_type") or "manual_entry").strip(),
                source_email_id=int(body.get("source_email_id")) if body.get("source_email_id") is not None else None,
                source_attachment_id=int(body.get("source_attachment_id")) if body.get("source_attachment_id") is not None else None,
                source_text_reference=str(body.get("source_text_reference") or "").strip() or None,
                confidence_score=float(body.get("confidence_score")) if body.get("confidence_score") is not None else None,
                requires_user_review=_coerce_request_bool(body.get("requires_user_review"), False),
                currency=str(body.get("currency") or "").strip() or None,
                unit_price=float(body.get("unit_price")) if body.get("unit_price") is not None else None,
                quantity=float(body.get("quantity")) if body.get("quantity") is not None else None,
                unit=str(body.get("unit") or "").strip() or None,
                moq=float(body.get("moq")) if body.get("moq") is not None else None,
                incoterm=str(body.get("incoterm") or "").strip() or None,
                origin_port=str(body.get("origin_port") or "").strip() or None,
                origin_airport=str(body.get("origin_airport") or "").strip() or None,
                destination_port=str(body.get("destination_port") or "").strip() or None,
                destination_airport=str(body.get("destination_airport") or "").strip() or None,
                lead_time_days=int(body.get("lead_time_days")) if body.get("lead_time_days") is not None else None,
                production_time_days=int(body.get("production_time_days")) if body.get("production_time_days") is not None else None,
                validity_date=str(body.get("validity_date") or "").strip() or None,
                payment_terms=str(body.get("payment_terms") or "").strip() or None,
                packaging=str(body.get("packaging") or "").strip() or None,
                technical_specs=json.dumps(body.get("technical_specs"), ensure_ascii=False) if body.get("technical_specs") is not None and not isinstance(body.get("technical_specs"), str) else (str(body.get("technical_specs")) if body.get("technical_specs") is not None else None),
                raw_response=str(body.get("raw_response") or "").strip() or None,
        ...
        return web.json_response({
            "object": "hermes.compras.quote.record",
            "rfq_batch_id": rfq_batch_id,
            "supplier_id": supplier_id,
            "quote_id": quote_id,
            "status": "stored",
    ...
    async def _handle_compras_calculate_sale_price(self, request: "web.Request") -> "web.Response":
        """POST /api/hermes/compras/pricing/calculate — store a sale price calculation."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        body, err = await self._read_json_body(request)
        ...
        try:
            ...
            supplier_id = int(body.get("supplier_id")) if body.get("supplier_id") is not None else None
            product_id = int(body.get("product_id")) if body.get("product_id") is not None else None
            quote_id = int(body.get("quote_id")) if body.get("quote_id") is not None else None
        except (TypeError, ValueError):
            return web.json_response(_openai_error("Invalid quote or RFQ identifiers", code="invalid_pricing_refs"), status=400)

        from hermes_compras_state import open_compras_db
        with open_compras_db() as db:
            calculation_id = db.calculate_sale_price(
                rfq_batch_id=rfq_batch_id,
                supplier_id=supplier_id,
                product_id=product_id,
                quote_id=quote_id,
                margin_rule_id=int(body.get("margin_rule_id")) if body.get("margin_rule_id") is not None else None,
                source_incoterm=str(body.get("source_incoterm") or "").strip() or None,
                sale_incoterm=str(body.get("sale_incoterm") or "").strip() or None,
                purchase_currency=str(body.get("purchase_currency") or "").strip() or None,
                sale_currency=str(body.get("sale_currency") or "").strip() or None,
                purchase_unit_price=float(body.get("purchase_unit_price")) if body.get("purchase_unit_price") is not None else None,
                quantity=float(body.get("quantity")) if body.get("quantity") is not None else None,
                international_freight=float(body.get("international_freight")) if body.get("international_freight") is not None else None,
                insurance=float(body.get("insurance")) if body.get("insurance") is not None else None,
                origin_charges=float(body.get("origin_charges")) if body.get("origin_charges") is not None else None,
                destination_charges=float(body.get("destination_charges")) if body.get("destination_charges") is not None else None,
                customs_clearance_cost=float(body.get("customs_clearance_cost")) if body.get("customs_clearance_cost") is not None else None,
                import_duties_estimated=float(body.get("import_duties_estimated")) if body.get("import_duties_estimated") is not None else None,
                taxes_estimated=float(body.get("taxes_estimated")) if body.get("taxes_estimated") is not None else None,
                inland_freight=float(body.get("inland_freight")) if body.get("inland_freight") is not None else None,
                warehouse_cost=float(body.get("warehouse_cost")) if body.get("warehouse_cost") is not None else None,
                financial_cost=float(body.get("financial_cost")) if body.get("financial_cost") is not None else None,
                other_costs=float(body.get("other_costs")) if body.get("other_costs") is not None else None,
                margin_type=str(body.get("margin_type") or "").strip() or None,
                margin_value=float(body.get("margin_value")) if body.get("margin_value") is not None else None,
                requires_user_approval=_coerce_request_bool(body.get("requires_user_approval"), True),
                approved_by=str(body.get("approved_by") or "").strip() or None,
                approved_at=float(body.get("approved_at")) if body.get("approved_at") is not None else None,
                created_at=created_at,
                updated_at=created_at,
            )
            calculation_row = db.fetchone("SELECT * FROM sale_price_calculations WHERE id = ?", (calculation_id,))

        return web.json_response({
            "object": "hermes.compras.pricing.calculate",
            "rfq_batch_id": rfq_batch_id,
            "supplier_id": supplier_id,
            "product_id": product_id,
            "quote_id": quote_id,
            "sale_price_calculation": calculation_row.data if calculation_row is not None else None,
        }, status=201)

    async def _handle_compras_build_proposal(self, request: "web.Request") -> "web.Response":
        """POST /api/hermes/compras/proposals/build — build the client proposal snapshot."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        body, err = await self._read_json_body(request)
        ...
        quote_id = int(body.get("quote_id")) if body.get("quote_id") is not None else None
        sale_price_calculation_id = int(body.get("sale_price_calculation_id")) if body.get("sale_price_calculation_id") is not None else None
        created_at = time.time()
        from hermes_compras_state import open_compras_db
        with open_compras_db() as db:
        ...
        return web.json_response({
            "object": "hermes.compras.proposal.build",
            "rfq_batch_id": rfq_batch_id,
            "proposal": proposal,
        })
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        routing = route_compras_turn(
            session.get("model") or self._model_name,
            user_message,
            conversation_history=history,
            system_prompt=system_prompt,
            has_image=_content_has_image(user_message),
        )
        result, usage = await self._run_agent(
            user_message=user_message,
            conversation_history=history,
            ephemeral_system_prompt=apply_compras_prompt_suffix(system_prompt, routing),
            session_id=session_id,
            gateway_session_key=gateway_session_key,
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        system_prompt = apply_compras_prompt_suffix(system_prompt, routing)
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        instructions = apply_compras_prompt_suffix(instructions, routing)
        model_name = routing.selected_model
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        ephemeral_system_prompt = apply_compras_prompt_suffix(instructions, routing)
        model_name = routing.selected_model
        loop = asyncio.get_running_loop()
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app.router.add_get("/api/hermes/compras/health", self._handle_compras_health)
            self._app.router.add_get("/api/hermes/compras/products", self._handle_compras_products)
            self._app.router.add_post("/api/hermes/compras/products", self._handle_compras_products)
            self._app.router.add_get("/api/hermes/compras/secoms", self._handle_compras_secoms)
            self._app.router.add_post("/api/hermes/compras/secoms", self._handle_compras_secoms)
            self._app.router.add_get("/api/hermes/compras/secoms/recommendations", self._handle_compras_secom_recommendations)
            self._app.router.add_get("/api/hermes/compras/rfq/dashboard", self._handle_compras_rfq_dashboard)
            self._app.router.add_get("/api/hermes/compras/rfq/worker/status", self._handle_compras_rfq_worker_status)
            self._app.router.add_post("/api/hermes/compras/rfq/worker/run-once", self._handle_compras_rfq_worker_run_once)
            self._app.router.add_post("/api/hermes/compras/rfq/sourcing", self._handle_compras_rfq_real_sourcing)
            self._app.router.add_post("/api/hermes/compras/rfq/prepare", self._handle_compras_prepare_rfq)
            self._app.router.add_post("/api/hermes/compras/rfq/approve", self._handle_compras_approve_rfq)
            self._app.router.add_post("/api/hermes/compras/rfq/inbound", self._handle_compras_record_inbound_email)
            self._app.router.add_post("/api/hermes/compras/quotes/record", self._handle_compras_record_supplier_quote)
            self._app.router.add_post("/api/hermes/compras/pricing/calculate", self._handle_compras_calculate_sale_price)
            self._app.router.add_post("/api/hermes/compras/proposals/build", self._handle_compras_build_proposal)
