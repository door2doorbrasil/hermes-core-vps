...
class WhatsAppBehaviorMixin:
    ...
    def _effective_reply_prefix(self) -> str:
        """Return the prefix to add to outgoing replies.

        Customer-facing WhatsApp replies are intentionally prefix-free.
        """
        return ""
    ...
    def _message_is_followup(self, body: str) -> bool:
        if not body:
            return False
        normalized = self._normalize_whitespace_text(body)
        return bool(
            re.search(
                r"\b(?:sim|nao|ok|mostra|me mostra|quero|quanto|preco|reuniao|reunioes|aula|aulas|entrevista|entrevistas|atendimento|atendimentos|trabalho|como funciona|quero ver|pode mandar|gostei)\b",
                normalized,
            )
        )
    ...
    def _should_process_message(self, data: Dict[str, Any]) -> bool:
        ...
        has_keyword_trigger = self._message_matches_mention_patterns(data)
        # WhatsApp uses pseudo-chats for Status updates (Stories) and
        # Channel/Newsletter broadcasts. These are not real conversations
        # and the agent should never reply to them — even in legacy mode
        ...
        if is_group:
            ...
            if not self._is_dm_allowed(sender_id):
                return False
            # DMs that pass the policy gate are processed by default, but if
            # mention_patterns were configured we use them as wake words here too
            # so ad leads can be filtered by keyword just like group traffic.
            if body.startswith("/"):
                return True
            ...
            if self._message_mentions_bot(data):
                return True
            if self._message_is_followup(body):
                return True
            if self._mention_patterns:
                return has_keyword_trigger
            return True
